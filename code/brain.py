...
class Brain(object):
    ...
    def __init__(self, mic, profile):
        ...
        print("[SmartDoor] loading encodings + face detector...")
        self.facial_data = pickle.loads(open(ENCODINGS, "rb").read())
        self.facial_detector = cv2.CascadeClassifier(CASCADE)
    ...
    def get_modules(cls):
        ...
        print("Looking for modules in: %s",
        ...
        for module in self.modules:
            for text in texts:
                if module.isValid(text):
                    print("'%s' is a valid phrase for module " +
                                       "'%s'", text, module.__name__)
                    try:
                        try:
                            module.handle(text, self.mic, self.profile,self.facial_data,self.facial_detector,self.sock)
                        except:
                        ...
                        return
        print("No module was able to handle any of these " +
                           "phrases: %r", texts)
